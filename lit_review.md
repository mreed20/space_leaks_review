...
Yang and Mazières work on *dynamic space limits* enables programmers to set an upper limit on the space usage of an arbitrary code block by placing it in a space-bounded container [@yang_dynamic_2013]. Their work could be used to implement automated regression testing for space leaks. For example, after *finding* a space leak, a developer could write a unit test executing the leaky code within a space-bounded container, whose bound would be exceeded while the leak is still present. The developer could verify potential leak fixes by running the unit test, which would crash if the code within exceeds its container's space limit. This approach would likely be more robust than an approach with weak pointers, which could only verify that a particular variable is not leaking. Unfortunately, dynamic space limits are not currently integrated into GHC mainline [@yang_dynamic_2013].
